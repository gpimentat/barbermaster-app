  ...
  const fetchProfile = async (userId: string) => {
    ...
    if (data) {
      ...
      const barber: Barber = {
        ...
        email: data.email,
        role: data.role,
        avatar: data.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.name || data.email)}&background=random&color=fff`,
        active: data.active,
      ...
      if (session?.user && session.user.id === userId) {
        ...
        const fallbackUser: Barber = {
          ...
          avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(user.user_metadata?.name || 'Administrador')}&background=random&color=fff`,
          active: true,
          commissionRate: 0,
          permissions: [],
          loginEnabled: true,
  ...
  const fetchBarbers = async (tenantId?: string) => {
    ...
    if (data) {
      const mapped = data.map((d: any) => ({
        ...
        email: d.email,
        role: d.role,
        avatar: d.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(d.name || d.email)}&background=random&color=fff`,
        active: d.active,
